...
__attribute__((inline)) unsigned char _SIM900_waitResponse(void);
...
{
    _SIM900_print("AT\r\n");
    return (_SIM900_waitResponse() == 1);
}

int SIM900_isReady(void)
{
    _SIM900_print("AT+CPIN?\r\n");
    return (_SIM900_waitResponse() == 2);
}
...
# 393 "WNB.c"
    Config();
     Lcd_Set_Cursor(1, 1);
    Lcd_Print_String("Waiting for msg..");
    while (1)
    {
        RecSMS();
    }
...
void ClearBuffers()
{
    Lcd_Clear();
    Lcd_Set_Cursor(1, 1);
    Lcd_Print_String(MsgMob);
    Lcd_Set_Cursor(2, 1);
    Lcd_Print_String(MsgTxt);
    strcpy(RcvdMsg, "");
    RcvdCheck = 0;
    RcvdConf = 0;
    index = 0;
